    ...
    for r in Restaurant.objects.all():
        try:
            if r.business_id:
                ...
                area = get_neighbourhood(restaurant_info['info']['location']['zip_code'])
                cuisine = get_cuisine(restaurant_info['info']['categories'])

                details = YelpRestaurantDetails(business_id=r.business_id, neighborhood = area,category = cuisine, price = restaurant_info['info']['price'], rating = restaurant_info['info']['rating'])
                details.save()

                logger.info("Yelp restaurant details successfully saved: {}".format(r.business_id))
            else:
                continue
        except Exception as e:
            logger.warning("Error while saving to table YelpRestaurantDetails: {}".format(r.business_id))
